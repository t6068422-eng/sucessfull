...
// Background Task: Delete expired tasks every minute
setInterval(() => {
  try {
    const now = new Date().toISOString();
    const expiredTasks = db.prepare("SELECT id FROM tasks WHERE expires_at IS NOT NULL AND expires_at < ?").all(now) as { id: number }[];
    
    if (expiredTasks.length > 0) {
      console.log(`[CLEANUP] Found ${expiredTasks.length} expired tasks. Deleting...`);
      ...
      deleteTx(expiredTasks.map(t => t.id));
      console.log(`[CLEANUP] Successfully deleted expired tasks: ${expiredTasks.map(t => t.id).join(', ')}`);
    }
  } catch (err) {
    console.error("[CLEANUP] Error during expired task cleanup:", err);
  }
}, 60000);
...
async function startServer() {
  ...
  app.all("/api/admin/tasks/delete", (req, res) => {
    ...
    console.log(`[SERVER] >>> ATTEMPTING DELETE for Task ID: ${id} (parsed: ${taskId})`);
    
    if (isNaN(taskId)) {
      console.error("[SERVER] Invalid Task ID received");
      return res.status(400).json({ success: false, message: "Invalid Task ID" });
    }

    try {
      // 1. Delete associated data
      const r1 = db.prepare("DELETE FROM user_tasks WHERE task_id = ?").run(taskId);
      // 2. Delete the task itself
      const r2 = db.prepare("DELETE FROM tasks WHERE id = ?").run(taskId);
      
      console.log(`[SERVER] Deletion results: user_tasks=${r1.changes}, tasks=${r2.changes}`);
      
      // 3. Final verification
      const check = db.prepare("SELECT id FROM tasks WHERE id = ?").get(taskId);
      if (check) {
        console.error("[SERVER] Task still exists after DELETE command!");
        return res.status(500).json({ success: false, message: "Database failed to remove record" });
      }

      res.json({ success: true, message: "Task deleted", changes: r2.changes });
    } catch (err: any) {
  ...
  app.all("/api/admin/tasks/bulk-delete", (req, res) => {
    ...
    console.log(`[SERVER] >>> BULK DELETE REQUEST for IDs: ${JSON.stringify(ids)}`);
    ...
    try {
      ...
      // Verification
      const remaining = db.prepare(`SELECT id FROM tasks WHERE id IN (${ids.map(() => '?').join(',')})`).all(...ids);
      if (remaining.length > 0) {
        console.error("[SERVER] Some tasks still exist after BULK DELETE:", remaining);
        return res.status(500).json({ success: false, message: "Database failed to remove some records", remaining: remaining.length });
      }
      
      res.json({ success: true, message: `Successfully deleted ${ids.length} tasks` });
    } catch (err: any) {
      console.error("[SERVER] Bulk Deletion Error:", err);
      res.status(500).json({ success: false, message: err.message });
    }
  });
  ...
  app.post("/api/games/play", (req, res) => {
    ...
    if (count >= 10) {
      return res.status(403).json({ message: "Daily limit reached (10 plays per day)" });
    }
